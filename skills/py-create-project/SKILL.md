...
### 1. DDD & Encapsulation Rules (from `py-domain-driven-design.md`)
...
### 2. Python Features & Coding Style (from `py-coding-style.md`)
...
### 3. Monadic Control Flow (from `py-architecture.md` & `py-coding-style.md`)
...
### 5. Centralized Error Handling (from `py-coding-style.md`)
- Define base domain exceptions and subclass specific errors (e.g. `BillingError` -> `ClientValidationError`).

### 6. Web Routers & Workers (from `py-coding-style.md`)
...
### 7. Unit Testing (from `py-testing.md`)
